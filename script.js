...
function loadData() {
  ...
  loadedItems.forEach(item => {
    // 期限が日付形式でない場合はスキップするなどの処理を加えても良い
...
// === 検索処理 (スケジュールテーブルの検索ロジックは無効化) ===
document.getElementById('searchBox').addEventListener('input', () => {
  const keyword = document.getElementById('searchBox').value.trim().toLowerCase();

  // スケジュールテーブルの検索は無効化（HTMLから検索フォームを移動したため）
  // const rows = document.querySelectorAll('#scheduleTable tr');
  // rows.forEach((row, index) => {
  //   if (index === 0) return;
  ...
  // 検索結果をリストの先頭に持ってくる処理は維持
  messageList.innerHTML = '';
  // 一致したものを先頭に表示、一致しないものは非表示（上記のループで処理済み）
